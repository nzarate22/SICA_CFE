...
@Component({
  selector: 'app-solicitar-ingreso',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule],
  templateUrl: './solicitar-ingreso.html',
  styleUrl: './solicitar-ingreso.scss',
})
export class SolicitarIngreso implements OnInit {
  rpe: string = '';
  nombre: string = '';
  password_1: string = '';
  password_2: string = '';
    ...
    this.solicitudes.obtenerFirmantes().subscribe({
      next: (res: any) => {
        ...
        if (res?.status === 'success' && res?.firmas) {
          ...
          this.tecnicoAutomatico = tec?.nombre || 'ING. GERARDO CASTILLA';
          this.tecnicoRpe = tec?.rpe || 'SIN RPE';

          const div = this.catalogoCompletoFirmas.find(
            (f: any) => f.puesto_tipo == '1'
          );
          this.divisionalAutomatico = div?.nombre || 'ING. HUGO';
          this.divisionalRpe = div?.rpe || 'SIN RPE';
        }
  ...
  procesarRegistro() {
    ...
    if (!this.rpe || !this.nombre || !this.password_1 || !this.password_2 || !this.jefeSeleccionado) {
      this.msgError = 'Por favor, rellene todos los campos obligatorios y seleccione a su superior.';
      return;
    }

    if (this.password_1 !== this.password_2) {
      this.msgError = 'Las contraseñas no coinciden. Verifique los campos.';
      return;
    }

    const regexPassword = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{}|;':",./<>?¿¡]).{8,}$/;

    if (!regexPassword.test(this.password_1)) {
      this.msgError = 'La contraseña debe tener al menos 8 caracteres, incluir una mayúscula, una minúscula, un número y un carácter especial.';
      return;
    }
    ...
    const datosControles = {
      rpe: this.rpe.trim().toUpperCase(),
      nombre: this.nombre.trim(),
      password_1: this.password_1,
      tecnico_firmante: this.tecnicoAutomatico,
      jefe_firmante: this.jefeSeleccionado,
      divisional_firmante: this.divisionalAutomatico
    };

    this.solicitudes.enviarSolicitud(datosControles).subscribe({
      next: (response) => {
        this.cargando = false;
        this.msgExito = 'Solicitud autorizada correctamente. Generando y descargando responsiva oficial...';

        this.descargarPdfResponsiva();
      ...
      error: (err) => {
        this.cargando = false;
        this.msgError = err.error?.message || 'Hubo un fallo al intentar conectar con el servidor.';
        this.cdr.detectChanges();
      }
    });
  }

  descargarPdfResponsiva() {
    const rpeUsuario = this.rpe || 'SIN RPE';
    const nombreUsuario = this.nombre || 'Usuario del Sistema';
